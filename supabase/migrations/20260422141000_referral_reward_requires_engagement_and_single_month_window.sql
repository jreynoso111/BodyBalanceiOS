...
set sign_in_count = case
  when coalesce(u.last_sign_in_at, u.created_at) is not null then greatest(coalesce(p.sign_in_count, 0), 1)
  else coalesce(p.sign_in_count, 0)
end
...
where reward_eligible is distinct from true;
...
begin
  ...
  on conflict (id) do update
    set sign_in_count = coalesce(public.profiles.sign_in_count, 0) + 1,
        updated_at = now()
  returning sign_in_count into v_next_count;
...
begin
  ...
  where r.inviter_user_id = auth.uid()
    ...
    and (
      ...
      or coalesce(invitee_profile.sign_in_count, 0) > 2
    );
...
begin
  ...
  if v_cycles_to_award > 0 and v_reward_window_open then
    v_reward_months := 1;
